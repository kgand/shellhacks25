...
# For very video we call the below once
# Main Agent gets Video from firestore and puts in MemoryBank
# Main Agent then calls sub agents BUT uses A2A and instead passes only the specific filtered data from the MemoryBank
...
class TranscriberAgent(ADKAgent):
    ...
    def __init__(self):
        super().__init__(
            "Transcriber", "Converts audio to text using speech recognition"
        )
        self.model = None
...
class ActionPlannerAgent(ADKAgent):
    ...
    def __init__(self):
        super().__init__(
            "ActionPlanner", "Extracts and plans action items from conversations"
        )
...
class RelationshipMinerAgent(ADKAgent):
    ...
    def __init__(self):
        super().__init__(
            "RelationshipMiner",
            "Identifies relationships between people in conversations",
        )
...
class MemoryWriterAgent(ADKAgent):
    ...
    def __init__(self):
        super().__init__(
            "MemoryWriter", "Writes processed memories to long-term storage"
        )
